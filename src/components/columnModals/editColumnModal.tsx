...
export function EditColumnModal({ isOpen, onClose, column, onSave }: EditColumnModalProps) {
    ...
    const handleSave = (e: React.FormEvent) => {
        e.preventDefault();
        if (column) {
            onSave({ ...column, title, color: selectedColor, description });
        }
        onClose();
    };
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div className="bg-gray-800 border border-slate-600 p-8 rounded-md w-96">
                <h2 className="text-xl font-bold text-white text-center mb-4">Editar Coluna</h2>
